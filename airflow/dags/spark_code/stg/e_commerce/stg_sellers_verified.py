...
olist_sellers_dataset = f's3a://lakehouse/raw/olist_sellers_dataset.csv'
df_sellers_raw = read_data_from_minio(
                                spark=spark,
                                path=olist_sellers_dataset
                                )

stg_sellers_verified = df_sellers_raw.select(
    col("seller_id"),
    col("seller_city"),
    col("seller_state"),
    lpad(col("seller_zip_code_prefix"), 5, "0").alias("seller_zip_code")
).withColumn("datadate", F.to_date(F.lit(datadate), "yyyy-MM-dd"))

write_iceberg_dynamic_partition(
    spark=spark,
    df=stg_sellers_verified,
    table_name='nessie.stg.stg_sellers_verified'
)
