...
datadate = get_args()
# ==========================================
# 5. TẠO BẢNG STG_PARTIES_VERIFIED (Customers & Sellers)
# Mục tiêu: Chuẩn hóa Zip Code và định danh khách hàng duy nhất [cite: 44, 46]
# ==========================================
print("Processing stg_parties_verified...")
olist_customers_dataset = f's3a://lakehouse/raw/olist_customers_dataset.csv'
df_customers_raw = read_data_from_minio(
                                spark=spark,
                                path=olist_customers_dataset
                                )

# Logic: Customer Unique ID là quan trọng nhất để phân tích hành vi
stg_customers_verified = df_customers_raw.select(
    col("customer_id"),          # ID giao dịch (dùng để join với Orders)
    col("customer_unique_id"),   # ID khách hàng thực (dùng để phân tích)
    col("customer_city"),
    col("customer_state"),
    # Đảm bảo Zip Code luôn có 5 chữ số (thêm số 0 ở đầu nếu thiếu) để join với bảng Geo
    lpad(col("customer_zip_code_prefix"), 5, "0").alias("customer_zip_code")
).withColumn("datadate", F.to_date(F.lit(datadate), "yyyy-MM-dd"))

write_iceberg_dynamic_partition(
    spark=spark,
    df=stg_customers_verified,
    table_name='nessie.stg.stg_customers_verified'
)
